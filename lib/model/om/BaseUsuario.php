...
abstract class BaseUsuario extends BaseObject  implements Persistent {
	...
	/**
	 * The value for the usu_per_id field.
	 * @var        int
	 */
	protected $usu_per_id;
	...
	/**
	 * Get the [usu_per_id] column value.
	 * 
	 * @return     int
	 */
	...
	/**
	 * Set the value of [usu_per_id] column.
	 * 
	 * @param      int $v new value
	 * @return     Usuario The current object (for fluent API support)
	 */
	public function setUsuPerId($v)
	{
		if ($v !== null) {
			$v = (int) $v;
		}

		if ($this->usu_per_id !== $v) {
			$this->usu_per_id = $v;
			$this->modifiedColumns[] = UsuarioPeer::USU_PER_ID;
		}

		if ($this->aPerfil !== null && $this->aPerfil->getPerId() !== $v) {
			$this->aPerfil = null;
		}
	...
	public function hydrate($row, $startcol = 0, $rehydrate = false)
	{
		try {
			...
			$this->usu_per_id = ($row[$startcol + 1] !== null) ? (int) $row[$startcol + 1] : null;
			$this->usu_login = ($row[$startcol + 2] !== null) ? (string) $row[$startcol + 2] : null;
			$this->usu_clave = ($row[$startcol + 3] !== null) ? (string) $row[$startcol + 3] : null;
			$this->usu_estado = ($row[$startcol + 4] !== null) ? (string) $row[$startcol + 4] : null;
			$this->resetModified();
	...
	public function ensureConsistency()
	{
		...
		if ($this->aPerfil !== null && $this->usu_per_id !== $this->aPerfil->getPerId()) {
			$this->aPerfil = null;
		}
	} // ensureConsistency
	...
	public function toArray($keyType = BasePeer::TYPE_PHPNAME, $includeLazyLoadColumns = true)
	{
		...
		$result = array(
			...
			$keys[1] => $this->getUsuPerId(),
			$keys[2] => $this->getUsuLogin(),
			$keys[3] => $this->getUsuClave(),
	...
	public function fromArray($arr, $keyType = BasePeer::TYPE_PHPNAME)
	{
		...
		if (array_key_exists($keys[1], $arr)) $this->setUsuPerId($arr[$keys[1]]);
		if (array_key_exists($keys[2], $arr)) $this->setUsuLogin($arr[$keys[2]]);
		if (array_key_exists($keys[3], $arr)) $this->setUsuClave($arr[$keys[3]]);
	...
	public function buildCriteria()
	{
		...
		if ($this->isColumnModified(UsuarioPeer::USU_PER_ID)) $criteria->add(UsuarioPeer::USU_PER_ID, $this->usu_per_id);
	...
	public function copyInto($copyObj, $deepCopy = false)
	{
		...
		$copyObj->setUsuPerId($this->usu_per_id);
	...
	{
		if ($v === null) {
			$this->setUsuPerId(NULL);
		} else {
			$this->setUsuPerId($v->getPerId());
		}
	...
	{
		if ($this->aPerfil === null && ($this->usu_per_id !== null)) {
			$this->aPerfil = PerfilPeer::retrieveByPk($this->usu_per_id);
	 ...
	 * related Prestadors from storage. If this Usuario is new, it will return
		...
		if ($this->collPrestadors === null) {
			if ($this->isNew()) {
			   $this->collPrestadors = array();
				...
				$criteria->add(PrestadorPeer::PRE_USU_ID, $this->usu_id);
	 ...
	 */
	public function getPrestadorsJoinRango($criteria = null, $con = null, $join_behavior = Criteria::LEFT_JOIN)
	{
		if ($criteria === null) {
			$criteria = new Criteria(UsuarioPeer::DATABASE_NAME);
		}
		...
		if ($this->collPrestadors === null) {
			if ($this->isNew()) {
				...
				$this->collPrestadors = PrestadorPeer::doSelectJoinRango($criteria, $con, $join_behavior);
			}
		} else {
			// the following code is to determine if a new query is
			// called for.  If the criteria is the same as the last
