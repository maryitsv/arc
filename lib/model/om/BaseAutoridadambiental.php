...
/**
 * Base class that represents a row from the 'autoridadambiental' table.
 *
 * 
 *
 ...
 */
abstract class BaseAutoridadambiental extends BaseObject  implements Persistent {
	 ...
	 * @var        AutoridadambientalPeer
	 */
	protected static $peer;

	/**
	 * The value for the aua_id field.
	 * @var        int
	 */
	protected $aua_id;

	/**
	 * The value for the aua_nombre field.
	 * @var        string
	 */
	protected $aua_nombre;
	...
	const PEER = 'AutoridadambientalPeer';

	/**
	 * Get the [aua_id] column value.
	 * 
	 * @return     int
	 */
	public function getAuaId()
	{
		return $this->aua_id;
	}

	/**
	 * Get the [aua_nombre] column value.
	 * 
	 * @return     string
	 */
	public function getAuaNombre()
	{
		return $this->aua_nombre;
	}

	/**
	 * Set the value of [aua_id] column.
	 * 
	 * @param      int $v new value
	 * @return     Autoridadambiental The current object (for fluent API support)
	 */
	public function setAuaId($v)
	{
		if ($v !== null) {
			$v = (int) $v;
		}
		...
		if ($this->aua_id !== $v) {
			$this->aua_id = $v;
			$this->modifiedColumns[] = AutoridadambientalPeer::AUA_ID;
		}
	...
	/**
	 * Set the value of [aua_nombre] column.
	 * 
	 * @param      string $v new value
	 * @return     Autoridadambiental The current object (for fluent API support)
	 */
	public function setAuaNombre($v)
	{
		if ($v !== null) {
			$v = (string) $v;
		}

		if ($this->aua_nombre !== $v) {
			$this->aua_nombre = $v;
			$this->modifiedColumns[] = AutoridadambientalPeer::AUA_NOMBRE;
		}
	...
	public function hydrate($row, $startcol = 0, $rehydrate = false)
	{
		try {
			...
			$this->aua_id = ($row[$startcol + 0] !== null) ? (int) $row[$startcol + 0] : null;
			$this->aua_nombre = ($row[$startcol + 1] !== null) ? (string) $row[$startcol + 1] : null;
			$this->resetModified();
			...
			return $startcol + 2; // 2 = AutoridadambientalPeer::NUM_COLUMNS - AutoridadambientalPeer::NUM_LAZY_LOAD_COLUMNS).

		} catch (Exception $e) {
			throw new PropelException("Error populating Autoridadambiental object", $e);
		}
	}
	...
	public function reload($deep = false, PropelPDO $con = null)
	{
		...
		$stmt = AutoridadambientalPeer::doSelectStmt($this->buildPkeyCriteria(), $con);
	...
	public function delete(PropelPDO $con = null)
	{
		...
		try {
			...
			foreach (sfMixer::getCallables('BaseAutoridadambiental:delete:pre') as $callable)
			{
			...
			if ($ret) {
				AutoridadambientalPeer::doDelete($this, $con);
				$this->postDelete($con);
				// symfony_behaviors behavior
				foreach (sfMixer::getCallables('BaseAutoridadambiental:delete:post') as $callable)
				{
	...
	public function save(PropelPDO $con = null)
	{
		...
		try {
			...
			// symfony_behaviors behavior
			foreach (sfMixer::getCallables('BaseAutoridadambiental:save:pre') as $callable)
			{
			...
			if ($ret) {
				...
				$this->postSave($con);
				// symfony_behaviors behavior
				foreach (sfMixer::getCallables('BaseAutoridadambiental:save:post') as $callable)
				{
				  call_user_func($callable, $this, $con, $affectedRows);
				}

				AutoridadambientalPeer::addInstanceToPool($this);
			} else {
				$affectedRows = 0;
			}
			$con->commit();
	...
	protected function doSave(PropelPDO $con)
	{
		...
		if (!$this->alreadyInSave) {
			...
			if ($this->isModified()) {
				if ($this->isNew()) {
					$pk = AutoridadambientalPeer::doInsert($this, $con);
					$affectedRows += 1; // we are assuming that there is only 1 row per doInsert() which
										 // should always be true here (even though technically
										 // BasePeer::doInsert() can insert multiple rows).

					$this->setAuaId($pk);  //[IMV] update autoincrement primary key

					$this->setNew(false);
				} else {
					$affectedRows += AutoridadambientalPeer::doUpdate($this, $con);
				}
	...
	protected function doValidate($columns = null)
	{
		if (!$this->alreadyInValidation) {
			...
			if (($retval = AutoridadambientalPeer::doValidate($this, $columns)) !== true) {
	...
	{
		$pos = AutoridadambientalPeer::translateFieldName($name, $type, BasePeer::TYPE_NUM);
	...
	public function getByPosition($pos)
	{
		switch($pos) {
			case 0:
				return $this->getAuaId();
				break;
			case 1:
				return $this->getAuaNombre();
				break;
			default:
				return null;
				break;
	...
	{
		$keys = AutoridadambientalPeer::getFieldNames($keyType);
		$result = array(
			$keys[0] => $this->getAuaId(),
			$keys[1] => $this->getAuaNombre(),
		);
		return $result;
	}
	...
	public function fromArray($arr, $keyType = BasePeer::TYPE_PHPNAME)
	{
		...
		if (array_key_exists($keys[0], $arr)) $this->setAuaId($arr[$keys[0]]);
		if (array_key_exists($keys[1], $arr)) $this->setAuaNombre($arr[$keys[1]]);
	}
	...
	public function buildCriteria()
	{
		...
		if ($this->isColumnModified(AutoridadambientalPeer::AUA_ID)) $criteria->add(AutoridadambientalPeer::AUA_ID, $this->aua_id);
		if ($this->isColumnModified(AutoridadambientalPeer::AUA_NOMBRE)) $criteria->add(AutoridadambientalPeer::AUA_NOMBRE, $this->aua_nombre);
	...
	public function buildPkeyCriteria()
	{
		...
		$criteria->add(AutoridadambientalPeer::AUA_ID, $this->aua_id);
	...
	/**
	 * Generic method to set the primary key (aua_id column).
	 *
	 ...
	 * objects.
	 *
	 * @param      object $copyObj An object of Autoridadambiental (or compatible) type.
	...
	public function copyInto($copyObj, $deepCopy = false)
	{
		...
		$copyObj->setAuaNombre($this->aua_nombre);
		...
		$copyObj->setAuaId(NULL); // this is a auto-increment column, so set to default value
	 ...
	 *
	 * @param      boolean $deepCopy Whether to also copy all rows that refer (by fkey) to the current row.
	 * @return     Autoridadambiental Clone of current object.
	 * @throws     PropelException
	 */
	 ...
	 *
	 * @return     AutoridadambientalPeer
	 */
	public function getPeer()
	{
		if (self::$peer === null) {
			self::$peer = new AutoridadambientalPeer();
		}
		return self::$peer;
	}
	 ...
	 * Otherwise if this Autoridadambiental has previously been saved, it will retrieve
	 * related Disposicionfinalresiduosaseos from storage. If this Autoridadambiental is new, it will return
		...
		if ($this->collDisposicionfinalresiduosaseos === null) {
			if ($this->isNew()) {
			   $this->collDisposicionfinalresiduosaseos = array();
				...
				$criteria->add(DisposicionfinalresiduosaseoPeer::DFR_AUTORIDAD_AUTONOMA_ID, $this->aua_id);
	 ...
	 * Otherwise if this Autoridadambiental is new, it will return
	 * an empty collection; or if this Autoridadambiental has previously
	 * been saved, it will retrieve related Disposicionfinalresiduosaseos from storage.
	 *
	 * This method is protected by default in order to keep the public
	 * api reasonable.  You can provide public methods for those you
	 * actually need in Autoridadambiental.
	 */
	...
	{
	  if (!$callable = sfMixer::getCallable('BaseAutoridadambiental:'.$method))
	  {
	    throw new sfException(sprintf('Call to undefined method BaseAutoridadambiental::%s', $method));
	  }
