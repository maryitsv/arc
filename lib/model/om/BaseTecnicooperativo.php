...
abstract class BaseTecnicooperativo extends BaseObject  implements Persistent {
	 ...
	 * If this collection has already been initialized with an identical Criteria, it returns the collection.
	 * Otherwise if this Tecnicooperativo has previously been saved, it will retrieve
	 * related Captacions from storage. If this Tecnicooperativo is new, it will return
	 * an empty collection or the current collection, the criteria is ignored on a new object.
	 *
	 * @param      PropelPDO $con
	 * @param      Criteria $criteria
		...
		if ($this->collCaptacions === null) {
			if ($this->isNew()) {
			   $this->collCaptacions = array();
				...
				$criteria->add(CaptacionPeer::CAPT_TOP_ID, $this->top_id);
	 ...
	 * actually need in Tecnicooperativo.
	 */
	public function getCaptacionsJoinFuenteenergia($criteria = null, $con = null, $join_behavior = Criteria::LEFT_JOIN)
	{
		if ($criteria === null) {
			$criteria = new Criteria(TecnicooperativoPeer::DATABASE_NAME);
		}
		...
		if ($this->collCaptacions === null) {
			if ($this->isNew()) {
				...
				$this->collCaptacions = CaptacionPeer::doSelectJoinFuenteenergia($criteria, $con, $join_behavior);
			}
		} else {
			// the following code is to determine if a new query is
			// called for.  If the criteria is the same as the last
